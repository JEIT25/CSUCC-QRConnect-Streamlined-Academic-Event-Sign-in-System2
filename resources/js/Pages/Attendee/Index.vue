                ...
                <table class="min-w-full table-auto bg-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <tbody class="text-gray-600 text-sm text-start">
                        <tr v-for="attendee in attendees" :key="attendee.id"
                            ...
                            <td class="py-3 px-6">{{ attendee.check_in ? new
                                Date(attendee.check_in).toLocaleTimeString() :
                                'Not Checked-in' }}</td>
                            <td class="py-3 px-6">{{ attendee.check_out ? new
                                Date(attendee.check_out).toLocaleTimeString()
                                : 'Not Checked-out' }}</td>
                            <td class="py-3 px-6 text-center">
                                <Link :href="`/events/${event.id}/attendees/${attendee.id}`" method="delete" as="button"
